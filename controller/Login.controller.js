const {getData: users,getAdmin:admins, selectData, selectDataAdmin} = require("../model/user.model");
...
const authenticateAdmin = async (email, password) => {
    ...
    const user = admins[0].find(user1 => {
        console.log(typeof user1.EMAIL_ID);
        console.log(typeof email);
        console.log(user1.EMAIL_ID, email); // add this line to print the values of email and user1.EMAIL_ID
        if(user1.EMAIL_ID.localeCompare(email.trim()) === 0) {
    ...
    console.log(user)
    if (user == null) {
        console.log("nulllllllllasss2222s")
        return false
    }
    try {
        let resB = await bcrypt.compare(password, user.PASSWORD);
...
const postAdmin=async (req, res) => {
    ...
    let bool = false;
    console.log(req.body)
    bool = await authenticateAdmin(req.body.email.trim(), req.body.password.trim());
    console.log("bool1");
    ...
    setTimeout(async () => {
        if (bool) {
            // console.log("login done")
            const result = await selectDataAdmin(req.body.email);
            res.json({
                STUDENT_ID:result.STUDENT_ID,
                NAME:result.NAME,
                EMAIL_ID:result.EMAIL_ID,
