...
function checkOrder() {
    ...
    db.get(selectQuery, [], (err, row) => {
        ...
        db.get(`
            SELECT *
            FROM orders
            WHERE item = ? AND status != 'Delivered'
        ...
        `, [row.item], (err, order) => {
            ...
            if (order.tobedelivered <= itemsToDeliver) {
                ...
                db.run(`
                    UPDATE orders SET status = 'Delivered',tobedelivered = ?, delivered = ?, deliverid = ?
                    WHERE deliverid = ?
                ...
                db.run(`
                    UPDATE dummyorders SET quantity = quantity - ?
                    WHERE item = ?
                ...
                db.run(`
                    UPDATE orders SET tobedelivered = tobedelivered -?, delivered = ?,  status = 'Partially Delivered'
                    WHERE deliverid = ?
...
function deleteDummyOrder(item) {
    db.run(`
        DELETE FROM dummyorders WHERE item = ? AND quantity = 0
    `, [item], (err) => {
        if (err) {
            console.error(err);
        }
